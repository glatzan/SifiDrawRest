...
class ImageGroupService @Autowired constructor(
    ...
    fun getImageGroup(imageGroupPath: String, loadImageData: Boolean, format : String = "png", minimize : Boolean = false): ImageGroup {
        ...
        if (folder.isDirectory) {
            ...
            val images = imageService.getImagesOfFolder(fixedFolderPath, loadImageData,format);

            if(minimize) {
                images.forEach {
                    it.layers.clear()
                    // reducing concurrency counter, this image should not be saved anymore
                    it.concurrencyCounter--;
                }
            }

            imageGroup.images.addAll(images)
            return imageGroup
        }
